...
def getQuantile(df):
    muito_baixa = df['nota_geral']*0.20
    baixa = df['nota_geral']*0.40
    media = df['nota_geral']*0.60
    alta = df['nota_geral']*0.80
    muito_alta = df['nota_geral']*0.100
    cinquenta = df['nota_geral']*0.50
...
def classes2_60_teste(df, bins):
    bins2 = [df['nota_geral'].min(),bins['media'], df['nota_geral'].max()]
    labels = ['Baixa', 'Alta']
    df['nota_geral'] = pd.cut(df['nota_geral'], bins2, labels = labels,include_lowest = True)
    df['nota_geral'].replace({'Baixa':1,'Alta':2},inplace=True) #privada
...
def classes2(df):
    ...
    df['nota_geral'] = pd.cut(df['nota_geral'], bins, labels = labels,include_lowest = True)
    df['nota_geral'].replace({'Baixa':1,'Alta':2},inplace=True) #privada
    return {'media':(metade+cinquenta)/2}
...
def classes3(df):
    ...
    return {'trinta':df['nota_geral'].quantile(.33),'sessenta':df['nota_geral'].quantile(.67)}
    
def classes3_teste(df,bins):
    # bins = [0.0,df['nota_geral'].quantile(.33), df['nota_geral'].quantile(.67), 1.0]
    bins2 = [df['nota_geral'].min(),bins['trinta'],bins['sessenta'], df['nota_geral'].max()]
    labels = ['Baixa', 'Média', 'Alta']
    df['nota_geral'] = pd.cut(df['nota_geral'], bins2, labels = labels,include_lowest = True)
    df['nota_geral'].replace({'Baixa':1,'Média':2, 'Alta':3},inplace=True) #privada
...
def classes5(df):
    ...
    return {'muito_baixa':muito_baixa,'baixa':baixa,'media':media, 'alta':alta}

def classes5_teste(df,bins):
    bins2 = [df['nota_geral'].min(),bins['muito_baixa'],bins['baixa'],bins['media'],bins['alta'], df['nota_geral'].max()]
    labels = ['Muito Baixa','Baixa', 'Média', 'Alta', 'Muito Alta']
    df['nota_geral'] = pd.cut(df['nota_geral'], bins2, labels = labels,include_lowest = True)
    df['nota_geral'].replace({'Muito Baixa':0,'Baixa':1, 'Média':2, 'Alta':3, 'Muito Alta':4},inplace=True) #privada
